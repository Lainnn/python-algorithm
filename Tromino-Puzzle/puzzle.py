...
class puzzle:
    ...
    def solve(self, block, left, right, bottom, topr):
        ...
        # Your code goes here:
        if (right - left) >= 1:
            self.fill_L(self.get_quadrant(block,left,right,bottom,topr),left,right,bottom,topr)
            cr, cc = (topr+bottom)//2, (right+left)//2
            print("cc")
            print(cc)
            print("cr")
            print(cr)
            if self.get_quadrant(block,left,right,bottom,topr) == 3:
            ...
            elif self.get_quadrant(block,left,right,bottom,topr) == 4:
                self.solve((block),cc+1,right,bottom,cr)
                self.solve((cc,cr),left,cc,bottom,cr)
                self.solve((cc+1,cr+1),cc+1,right,cr+1,topr)
                self.solve((cc,cr+1),left,cc,cr+1,topr)
            elif self.get_quadrant(block,left,right,bottom,topr) == 2:
    ...
    def get_quadrant(self, block, left, right, bottom, topr):
        # Your code goes here
        cr, cc = (topr+bottom)//2, (right+left)//2
        if block[0] <= cc and block[1] <= cr:
            return 3
        elif block[0] > cc and block[1] <= cr:
            return 4
        elif block[0] <= cc and block[1] > cr:
            return 2
        elif block[0] > cc and block[1] > cr:
